...
#     help = 'Extract questions and options from the first page of PDF using PyMuPDF and print to the console.'
...
#             left_column_text = first_page.get_text("text", clip=left_bbox)
#             right_column_text = first_page.get_text("text", clip=right_bbox)
...
class Command(BaseCommand):
    help = 'Extract questions and options from the first page of PDF, delete old data, and populate the database.'
    ...
    def handle(self, *args, **kwargs):
        ...
        def extract_questions_from_text(text):
            questions_list = []
            expected_question_number = 1  # Start with the first expected question number
            # Match question and options pattern
            question_pattern = r"(\d+)\.\s+(.*?)(?=\s+[A-D]\.)"  # Match question until next option
            options_pattern = r"A\.\s*(.*?)\s+B\.\s*(.*?)\s+C\.\s*(.*?)\s+D\.\s*(.*?)(?=\s*\d+\.)"  # Match options until next question
            ...
            for i, (q_num, q_text) in enumerate(questions):
                ...
                while expected_question_number < q_num:
                    self.stdout.write(self.style.WARNING(f"Warning: Question {expected_question_number} is missing."))
                ...
                # Move to the next expected question number
                expected_question_number += 1
        ...
        # Delete all existing questions in the database

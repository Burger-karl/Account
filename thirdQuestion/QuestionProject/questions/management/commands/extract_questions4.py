...
#     help = 'Extract questions and options from the fourth page of PDF using PyMuPDF and print to the console.'
...
#             fourth_page = pdf_document[3]
#             width, height = fourth_page.rect.width, fourth_page.rect.height
...
#             left_column_text = fourth_page.get_text("text", clip=left_bbox)
#             right_column_text = fourth_page.get_text("text", clip=right_bbox)
...
class Command(BaseCommand):
    help = 'Extract questions 44 to 50 and options from the fourth page of PDF, delete old data, and populate the database.'
    ...
    def handle(self, *args, **kwargs):
        ...
        def extract_questions_from_text(text):
            ...
            for i, (q_num, q_text) in enumerate(questions):
                ...
                # Only process questions 44 to 50
                if 44 <= q_num <= 50:
                    # Ensure that we have options available for the corresponding question
                    if i < len(options):
                        a, b, c, d = options[i] if len(options) > i else ("", "", "", "")
                        question_data = {
                            ...
                            "question_text": f"{q_num}. {q_text.strip()}",  # Add number to the question text
                            "option_a": a.strip(),
                            "option_b": b.strip(),
                            "option_c": c.strip(),
                            "option_d": d.strip()
        ...
        def extract_questions(pdf_path):
            ...
            # Extract text from both columns on the fourth page
            left_column_text = fourth_page.get_text("text", clip=left_bbox)
            right_column_text = fourth_page.get_text("text", clip=right_bbox)
        ...
        # Extract questions from the fourth page
        questions = extract_questions(pdf_path)
        ...
        self.stdout.write(self.style.SUCCESS('Successfully extracted questions 44 to 50 and populated the database.'))
